...
# def aggregate_feature(cluster, child_name, reducer_feature_name):
#     (reducer_name, feature_name) = reducer_feature_name.split(":", 2)
#     parser = FeatureParser(cluster, child_name, feature_name)
...
def aggregate_features(cluster, child_name, reducer_feature_names):
    parser = FeatureParser(cluster, child_name, None)
    fc = parser.get_cluster_feature_collection()
    ...
    for reducer_feature_name in reducer_feature_names:
        ...
        if len(features) > 0:
            reducer.add_values(features)
            agg_feature = reducer.get_aggregate_feature(feature_name.replace(".", "_"))
            if agg_feature:
                if not isinstance(agg_feature, list):
